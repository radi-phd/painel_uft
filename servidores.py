...
# Versão Cloud-Safe: lê abas por índice, paths absolutos,
# requirements validados, encoding defensivo
# ============================================================
...
# PALETA
# ============================================================
P  = "#0D2B4E"; S  = "#1A4A7A"; A  = "#1E6FBF"; D  = "#C8A84B"
FG = "#F4F6F9"; BD = "#DDE3EC"; TL = "#6B7A99"
AL = "#D95F2B"; OK = "#1B7A4A"
...
BL = dict(paper_bgcolor="white", plot_bgcolor="white",
...
# ============================================================
# CAMINHO DO ARQUIVO — robusto para Cloud e local
# ============================================================
...
def carrega():
    ...
    try:
        # Lê por ÍNDICE de aba (0=técnico, 1=docente) — evita problema
        # de encoding com nomes acentuados em servidores Linux
        xl  = pd.ExcelFile(ARQUIVO, engine="openpyxl")
        nomes = xl.sheet_names          # descobre os nomes reais
        tec = xl.parse(nomes[0], dtype=str)   # primeira aba
        doc = xl.parse(nomes[1], dtype=str)   # segunda aba
...
# ── Carrega e trata erro com mensagem clara ──────────────────
df_raw, erro = carrega()
if erro:
    st.error(f"❌ {erro}")
    st.info("Verifique se `perfil.xlsx` está na raiz do repositório GitHub "
            "e se `requirements.txt` contém `openpyxl`.")
    st.stop()
...
st.markdown(
    ...
    + kpi("✅",f"{ativos:,}",     "Ativos",        OK,f"{ativos/max(total,1)*100:.1f}%")
...
with tab1:
    ...
    with c1:
        ...
        fig = px.bar(tg,x="Qtd",y="TIT_GRUPO",color="CATEGORIA",orientation="h",
                     barmode="stack",height=380,text_auto=True,
    ...
    fig_g = go.Figure(go.Indicator(
        ...
        gauge={"axis":{"range":[0,5],"tickwidth":1,"tickcolor":P},
               ...
               "steps":[{"range":[0,1],"color":"#F0F4FA"},{"range":[1,2],"color":"#C8D8EE"},
                         {"range":[2,3],"color":"#93B5D8"},{"range":[3,4],"color":"#4A90D9"},
...
with tab5:
    ...
    with d1:
        ...
        fig = px.pie(dv,names="Status",values="Qtd",hole=0.58,height=320,
                     color="Status",color_discrete_map={"Com desvio":AL,"Sem desvio":OK})
        fig.update_traces(texttemplate="<b>%{label}</b><br>%{value:,} (%{percent:.1%})",
                          textposition="outside",textfont_size=12)
        fig.update_layout(**BL)
        card(fig)
